...
// ============================================================
// 3. جلب البيانات (محدث لإرسال الباسورد وإزالة الفواصل)
// ============================================================
    ...
    .then((data) => {
      ...
      if (Array.isArray(data)) {
        customers = data.slice(1).map((row, i) => ({
          ...
          name: row[1],
          // ✅ إزالة الفاصلة العلوية من رقم الهاتف
          phone: String(row[2] || "").replace(/'/g, "").trim(),
          address: row[3] || "---",
          notes: row[4] || "",
        }));
      }
    ...
    .then((data) => {
      ...
      if (Array.isArray(data)) {
        ...
        orders = data.slice(1).map((row) => ({
          // ✅ إزالة الفاصلة العلوية من رقم المسلسل (ID)
          id: String(row[0] || "").replace(/'/g, "").trim(),
          client: row[1],
          // ✅ إزالة الفاصلة العلوية من رقم الهاتف
          phone: String(row[2] || "").replace(/'/g, "").trim(),
          address: row[3] || "---",
...
function updateDashboardUI() {
  ...
  if (mainBody) {
    if (customers.length === 0) {
      mainBody.innerHTML = `<tr><td colspan="6" style="text-align:center;">${t("no_results")}</td></tr>`;
    } else {
        ...
        .map(
          (c, i) => `
                <tr onclick="openCustomerProfile(${c.id})" title="اضغط لعرض البروفايل" style="cursor:pointer;">
                    ...
                    <td>${c.date}</td>
                    <td>${c.name}</td>
                    <td dir="ltr" style="font-weight:bold;">${c.phone}</td>
                    <td>${c.address}</td>
                    <td>${c.notes || '---'}</td>
                </tr>
            `,
...
function calculateTotalSales() {
  ...
  // ✅ إزالة الأصفار العشرية المزعجة
  salesEl.innerHTML = `${total.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} <span style="font-size:0.6em">AED</span>`;
}
...
function addNewOrderToTracking(
  ...
  const newOrder = {
    id: smartID,
    client: clientName,
    phone: clientPhone.replace(/'/g, ""), // ✅ تنظيف محلي
...
function renderOrdersTable(filter = "all") {
  ...
  displayOrders.forEach((order) => {
    ...
    // ✅ إزالة الأصفار العشرية المزعجة
    let formattedPrice = parseFloat(order.total).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
    ...
    if (filtered.length === 0)
      ...
      mainBody.innerHTML = [...filtered]
        ...
        .map(
          (c, i) => `
            <tr onclick="openCustomerProfile(${c.id})" title="اضغط لعرض البروفايل" style="cursor:pointer;">
                <td>${filtered.length - i}</td><td>${c.date}</td><td>${c.name}</td><td dir="ltr">${c.phone}</td><td>${c.address}</td><td>${c.notes || '---'}</td>
            </tr>`,
        )
        .join("");
  }
...
function openCustomerProfile(clientId) {
  ...
  if (clientOrders.length === 0) {
    ...
    [...clientOrders].reverse().forEach((order) => {
      ...
      // ✅ التعديل هنا لجدول المعاملات
      historyBody.innerHTML += `
                <tr>
                    <td>${formatDateFromID(order.id)}</td>
                    <td>${order.details || "---"}</td> <td>${order.type}</td>
                    <td style="color:#2ecc71;">${paid.toLocaleString('en-US', { maximumFractionDigits: 2 })}</td> <td style="color:var(--gold); font-weight:bold;">${amount.toLocaleString('en-US', { maximumFractionDigits: 2 })}</td> <td><span style="color:${statusColor}; border:1px solid ${statusColor}; padding:2px 8px; border-radius:10px; font-size:0.8rem;">${statusText}</span></td>
                </tr>
            `;
    });
  }
  ...
  document.getElementById("p-total-spent").innerText =
    totalSpent.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
